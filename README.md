...
1. Interfejsy VLAN (Bramy dla sieci)  
...
1. DHCP (Żeby komputer dostał IP)
...
1. Komunikacja Serwer - Stacja:
   Ping bezpośrednio ze stacji 10.27.20.15  na adres serwera
   ...
   jest OK.
    ...
    W WinBox, IP -\> ARP. Powinieneś tam widzieć:
...
1. Polecenia:
    ...
    ```bash
    /ip pool add name=pool2 ranges=10.27.20.10-10.27.20.15
    ```

    ```bash
    /ip dhcp-server add address-pool=pool2 interface=vlan2 name=dhcp2
    ```
    ...
    ```bash
    /ip dhcp-server network add address=10.27.20.0/24 dns-server=10.27.30.3 gateway=10.27.20.1 netmask=24
    ```
